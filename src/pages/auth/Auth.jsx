...
//     //     (state) => state.auth
//     // );
//     const { user, userPersonalInfo, isLoading, isLoggedIn } = useSelector((state) => state.auth);
//     console.log(userPersonalInfo, "userPersonalInfo in Authentication")
...
const Auth = () => {
    ...
    useEffect(() => {
        if (isLoggedIn && fetchedProfile) {
            if (fetchedProfile?.profileType === 'Admin') {
                navigate("/admin-dashboard");
            } else {
                navigate("/home");
            }
        }
    }, [isLoggedIn, fetchedProfile, navigate]);
